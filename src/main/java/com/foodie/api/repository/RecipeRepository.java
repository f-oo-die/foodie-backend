...
public interface RecipeRepository extends JpaRepository<Recipe, Long> {
  ...
  @Query(value = "SELECT distinct r.id, r.title, r.preparation, r.num_of_calories, r.type_of_meal, r.calorie_status, r.count FROM recipes r, recipes_nutrition_issues n WHERE r.type_of_meal = ?1 AND r.calorie_status = ?2 AND n.nutrition_issues_id IN (?3) ORDER BY count LIMIT 1", nativeQuery = true)
